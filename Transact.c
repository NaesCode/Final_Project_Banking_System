...
int verify_dep_amount(float money, float amount)
{
	if(amount > 0)
		return 1;
	else
...
void deposit(char *username, char *passcode, int *loggedIn)
{
    ...
    if (*loggedIn == 1)
    {
        ...
        invalid_input_catch:
            ...
            printf("\n\t\t\t\t\tAmount: ");
            if (scanf("%f", &depositAmount) == 0 || verify_dep_amount(money, depositAmount) == 0)  //checks if the input is truly a float
            {
            	fflush(stdin);
            	system("cls");
            	logo();
...
void money_transfer(char *username, char *passcode, int *loggedIn)
{
    ...
    int status = 0;
    float record_transact_transfer = 0;
        		...
        		case '1':
        			{
						        ...
						        record_transact_transfer = amount;
        						record_transaction(username, "Transfer" , record_transact_transfer);
        						record_transaction(recipient_username, "Transfered" , record_transact_transfer);
...
void bal_inquire(char *username, char *passcode, int *loggedIn)
{
